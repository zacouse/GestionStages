...
CREATE PROC pAddSetCreateStage(@IDMilieuStage_IN INT, @Titre_IN Varchar(100),@Description_IN Varchar(1000),
...
CREATE PROC pAddSetUpdateStage(@IDStage_IN INT, @IDMilieuStage_IN INT, @Titre_IN Varchar(100),@Description_IN Varchar(1000),
@NbPoste_IN INT, @Statut_IN TINYINT, @PeriodeTravail_IN TINYINT, @NbHeureSemaine_IN INT, @DateDebut_IN DateTime, @DateFin_IN DateTime, @Etat_IN Bit, @DateHeureCreation DateTime)
AS
BEGIN
UPDATE Stage ([IDMilieuStage], [Titre], [Description], [NbPostes], [Statut], [PeriodeTravail], [NbHeureSemaine], [DateDebut], [DateFin], [Etat], [DateHeureCreation], [DateHeureModification])
...
[DateDebut] = @DateDebut_IN, [DateFin] = @DateFin_IN,[Etat] = @Etat_IN, [DateHeureCreation] = @DateHeureCreation, [DateHeureModification] = GETDATE() 
